...
def add_sidebar():
    ...
    data = get_clean_data() #get data for sidebar sliders

    # create sliders for each feature
    # slider labels and feature names are associated for slider data
    slider_labels = [
    ...
    # key : value
    # column name : current value of slider
    input_dict = {}
    
    # add sliders for each feature
    # the label will be the slider label above with the key being the feature name in the data set 
...
def get_scaled_values(input_dict):
    # scale values between 0 and 1 by using the maximum and minimum values in the dataset
    # closer to min val = closer to 0, closer to max val = closer to 1
    data = get_clean_data()
    ...
    for key, value in input_dict.items():
        max_val = X[key].max()
        min_val = X[key].min()
        scaled_value = (value - min_val) / (max_val - min_val) # scale value between 0 and 1
        scaled_dict[key] = scaled_value
    
    return scaled_dict

def get_radar_chart(input_data): 
    # data set has 3 inherent series: mean, se, worst
    # multi trace radar chart will map these 3 series across the 10 features in the data set for each series
    ...
    # add trace for mean series
    fig.add_trace(go.Scatterpolar(
        # r contains the values from our input_data dictionary 
        r=[
...
def add_predictions(input_data): # input is a dict with key : value pairs 
    ...
    st.write("Probability of being benign: ", model.predict_proba(input_array_scaled)[0][0]) # probability of being benign
    st.write("Probability of being malignant: ", model.predict_proba(input_array_scaled)[0][1]) # probability of being malignant
    st.write("This app can assist medical professionals in making a diagnosis, but should not be used as a substitute for a professional diagnosis.") #disclaimer

def main():
    # initialize the app, basic page configuration
    st.set_page_config(
    ...
    # title of web page with description and ideal use case of application
    with st.container():
        st.title("Breast Cancer Predictor")
        st.write("Use this app with the help of your cytosis lab to diagnose breast cancer from your tissue samples. This app predicts using a machine learning model whether a breast mass is benign or malignant based on the measurements it recieves from your cytosis lab. You can also update the measurements by hand using the sliders in the sidebar.")

    # create main page content with 2 columns
